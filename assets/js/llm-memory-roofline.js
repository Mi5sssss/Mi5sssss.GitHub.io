(function () {
  ...
  function computeDerived() {
    ...
    var parts = state.placementPolicy === "kv_first"
      ? [{ key: "kv", label: "KV Cache", sizeGiB: kvTotalGiB }, { key: "weights", label: "Weights", sizeGiB: weightsGiB }, { key: "scratch", label: "Scratch", sizeGiB: scratchGiB }]
      : [{ key: "weights", label: "Weights", sizeGiB: weightsGiB }, { key: "kv", label: "KV Cache", sizeGiB: kvTotalGiB }, { key: "scratch", label: "Scratch", sizeGiB: scratchGiB }];
  ...
  function placementHtml(derived) {
    ...
    return rows.map(function (row) {
      ...
      return '<div class="lmr-part-card"><h4>' + escapeHtml(row.name) + '</h4><div class="lmr-part-grid">' + tiles + "</div></div>";
  ...
  function rulesHtml() {
    return '<div class="lmr-rule-stack">' + RULE_SHIFTS.map(function (item) {
      return '<div class="lmr-rule-card"><h4>' + escapeHtml(item.title) + '</h4><p>' + escapeHtml(item.body) + "</p></div>";
    }).join("") + "</div>";
  }
